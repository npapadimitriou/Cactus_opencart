...
class Product extends \Opencart\System\Engine\Controller {
	...
	public function notify() {
		...
		print($json);
		// Retrieve email and product ID from request
		$email = $json['email'];
		$product_id = $json['product_id'];
	
		// Basic validation
		if (filter_var($email, FILTER_VALIDATE_EMAIL) && !empty($product_id)) {
			// Load the model
			$this->load->model('catalog/product');
	
			// Save to database (for simplicity, assuming a separate notification table)
			// $this->db->query("INSERT INTO " . DB_PREFIX . "product_notifications SET product_id = '" . (int)$product_id . "', email = '" . $this->db->escape($email) . "'");
	
			// Send success message
			$this->response->setOutput(json_encode(['message' => 'You will be notified when this product is back in stock!']));
		} else {
			// Send error message
			$this->response->setOutput(json_encode(['message' => 'Invalid email or product ID.']));
		}
	}
